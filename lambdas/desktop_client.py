"""
Shared desktop client for all Coldbones Lambda functions.

Resolves the LM Studio endpoint from SSM Parameter Store:
...
The desktop exposes LM Studio via Tailscale Funnel — no open home-router ports,
...
def is_desktop_alive() -> bool:
    """
    Return True if LM Studio's /v1/models endpoint responds within
...
def get_openai_client(timeout: float = 120.0) -> Tuple[OpenAI, str]:
    """
    Return (OpenAI-compatible client, model_name) pointed at desktop LM Studio.
