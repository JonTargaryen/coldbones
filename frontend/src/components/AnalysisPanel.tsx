...
export function AnalysisPanel({ result, isAnalyzing, currentFileName, error, elapsedMs }: AnalysisPanelProps) {
  ...
  return (
    <div className="analysis-panel has-result" role="region" aria-label={t.summary}>
          ...
          >
            ...
            <h3>
              ...
              <span className="reasoning-meta">
                {result.reasoningTokenCount.toLocaleString()} {t.tokens}
              </span>
            </h3>
          </button>
          {reasoningOpen && (
      ...
      {result.extractedText && result.extractedText !== 'No text detected.' && result.extractedText !== t.noTextDetected && (
        <div className="result-section">
      ...
      <div className="result-meta">
        ...
        <span>{t.processedIn((result.processingTimeMs / 1000).toFixed(1))}</span>
